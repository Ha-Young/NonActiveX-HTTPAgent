...
using Mois_LocalWebServer_TEST.CONTROL;
using Mois_LocalWebServer_TEST.LOGIC;
using System;
...
//              차세대 주민등록정보시스템이 웹기반으로 바뀌면서 
//              기존의 ActiveX형태보다는 NonActiveX 형태로 도입을 원함.
//              NonActiveX방식을 위하여 HTTP(AJAX) 통신방식의 Local Web Server를 제작함.
//              As-Is는 본사 도입X. 각 프로그램별 다른 회사 제작. Delphi에 DLL Import하여 사용.
...
{
    delegate void Receive_WEB(HttpListenerRequest req, HttpListenerResponse res, HYWebServer server);
    public delegate void Send_WEB(AJAX_Response sendData, HYWebServer.TokenData token);
    delegate void Receive_EXE(WinMessage_ClientToServer receiveData, HYWebServer.TokenData token);
    delegate void Send_EXE(WinMessage_ServerToClient sendData);

    public partial class HYWebServer : Form
    {
        ...
        public static Send_WEB sendToWEB = new Send_WEB(WEB.Send);
        ...
        public class TokenData
        {
            ...
            public TokenData(AJAX_Request req, HttpListenerResponse res)
            {
        ...
        private void Mois_LocalWebServer_Load(object sender, EventArgs e)
        {
            Log.Instance.Info("############################################################################");
            Log.Instance.Info("######################  Mois Local HTTP Web Server 프로그램 실행  #####################");
            Log.Instance.Info("############################################################################");
        ...
        private void Mois_LocalWebServer_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            Start_Process(EXEKIND.TestScan, "", null, this.Handle);
        }
        ...
        public void Stop_Server()
        {
            ...
            Log.Instance.Info("## Mois Local HTTP Web Server Stop Success ##");
        }
        ...
        private void Exit_Server()
        {
            Stop_Server();
            Log.Instance.Info("############################################################################");
            Log.Instance.Info("######################  Mois Local HTTP Web Server 프로그램 종료  #####################");
            Log.Instance.Info("############################################################################");
            ...
            try
            {
                switch (m.Msg)
                {
                    case Win32API.WM_COPYDATA:
                        ...
                        //WEB_EXE_TranseToken
                        HYWebServer.TokenData token = WEB_EXE_TranseTokens[receiveData.PROCESS_KIND];
